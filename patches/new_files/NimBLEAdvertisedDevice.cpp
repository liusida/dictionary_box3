...
uint8_t NimBLEAdvertisedDevice::findAdvField(uint8_t type, uint8_t index, size_t* data_loc) const {
    ...
    while (length > 2) {
        // Safety check: make sure we still point inside the buffer
        if (data >= m_payload.size()) {
            break; // stop parsing, malformed payload
        }

        const ble_hs_adv_field* field =
            reinterpret_cast<const ble_hs_adv_field*>(&m_payload[data]);

        // field->length is the number of *following* bytes, so require at least 1 + length
        if (field->length + 1 > length) {
            break; // malformed, would run past the end
        }

        if (field->type == type ||
            (type == BLE_HS_ADV_TYPE_COMP_NAME && field->type == BLE_HS_ADV_TYPE_INCOMP_NAME)) {
    ...
    if (data_loc != nullptr && count > index && data < m_payload.size()) {
        *data_loc = data;
    }
