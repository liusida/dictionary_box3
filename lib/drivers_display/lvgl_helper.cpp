...
static void handleFunctionKeyEvent(const FunctionKeyEvent &ev) {
  ...
  case FunctionKeyEvent::Escape:
    ...
    if (s_onFunctionKeyIn)
      s_onFunctionKeyIn(ev.type);
    break;
  default:
    break;
  }
...
void lvglSetFunctionKeyCallbacks(const FunctionKeyCallback &onFunctionKeyIn) { s_onFunctionKeyIn = onFunctionKeyIn; }
