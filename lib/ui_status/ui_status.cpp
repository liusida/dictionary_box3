...
bool StatusOverlay::initialize() {
  ...
  container_ = lv_obj_create(null_screen_); // setting an object's parent to NULL would create a new screen, and attaching won't work, so we make the
                                            // container a child of the null screen before attaching to any screen.
  if (!container_) {
...
bool StatusOverlay::isVisible() const { return visible_; }
...
void StatusOverlay::updateWiFiStatus(WiFiState state, const String &ssid) {
  ...
  if (initialized_) {
    updateWiFiIndicator();
    const char *stateStr = (state == WiFiState::None) ? "none" : (state == WiFiState::Ready) ? "ready" : "working";
...
void StatusOverlay::updateAudioStatus(AudioState state, const String &track) {
  ...
  if (initialized_) {
    ...
    const char *stateStr = (state == AudioState::None) ? "none" : (state == AudioState::Ready) ? "ready" : "working";
...
void StatusOverlay::setAnimationDuration(uint16_t duration) { animationDuration_ = duration; }

bool StatusOverlay::isAttached() const { return attachedScreen_ != nullptr; }
