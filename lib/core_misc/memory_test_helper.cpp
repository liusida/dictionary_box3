...
bool checkForMemoryLeaks(const char *testName, const MemoryState *initialState, const MemoryState *currentState, uint32_t threshold) {
  ...
  ESP_LOGI("MemoryTest", "Heap: %u -> %u (delta: %d bytes)", initialState->freeHeap, currentState->freeHeap, heapDelta);
  ESP_LOGI("MemoryTest", "PSRAM: %u -> %u (delta: %d bytes)", initialState->freePsram, currentState->freePsram, psramDelta);
  ESP_LOGI("MemoryTest", "Min Free Heap: %u -> %u", initialState->minFreeHeap, currentState->minFreeHeap);
...
bool checkMemoryUsage(const char *testName, uint32_t initialHeap, uint32_t initialPsram, uint32_t threshold) {
  ...
  ESP_LOGI("MemoryTest", "[%s] Memory: Heap %u->%u (%d), PSRAM %u->%u (%d)", testName ? testName : "test", initialHeap, currentHeap, heapDelta,
...
void setUpMemoryMonitoring() { recordMemoryState(&g_initialMemoryState); }
