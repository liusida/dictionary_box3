 ...
 */
bool checkForMemoryLeaks(const char *testName, const MemoryState *initialState, const MemoryState *currentState,
 ...
 * @return true if no significant leaks detected, false if leaks found
 */
bool checkMemoryUsage(const char *testName, uint32_t initialHeap, uint32_t initialPsram, uint32_t threshold = MEMORY_LEAK_THRESHOLD_DEFAULT);
